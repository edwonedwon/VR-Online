...
namespace EasyEditor
{
	public class Utils {
        ...
        public static ScriptInterface GetScriptInterfaceFor(Type monoType)
        {
            ScriptInterface result = null;

            string[] searchResults = AssetDatabase.FindAssets("t:ScriptInterface");
            foreach (string searchResult in searchResults)
            {
                string path = AssetDatabase.GUIDToAssetPath(searchResult);
                ScriptInterface aScriptInterface = AssetDatabase.LoadAssetAtPath(path, typeof(ScriptInterface)) as ScriptInterface;
                if (aScriptInterface.renderedScript == null)
                {
                    Debug.Log("the script is null");
                }
                if (aScriptInterface.renderedScript.GetClass() == monoType)
                {
                    result = aScriptInterface;
                }
            }
